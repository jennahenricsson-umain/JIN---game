...
function render() {
    ...
    if (gameState === "menu") {
        ...
        if (
            Date.now() - menuEnteredAt > (idleLoop ? 10000 : 15000) &&
        ...
        if (gameMode === "single") {
            ...
            if (!introActive) {
                const title = overlay.querySelector(
                    ".scene-text--onboarding-title"
                );
                if (title) title.classList.add("onboarding-title--up");
            }

            if (
                (g1 === "Thumb_Down" && c1 >= 0.7) ||
                (g2 === "Thumb_Down" && c2 >= 0.7)
            ) {
                overlay.innerHTML = "";
                particles.innerHTML = "";
                gameMode = "single";
            ...
            if (p1done && !p2done) {
                ...
                if (!overlayP2.querySelector(".scene-text--waiting")) {
                    overlayP2.innerHTML =
                        '<p class="scene-text scene-text--waiting" style="left: 25%">Waiting for player 2...</p>';
                }
            }
            ...
            if (!p1done && p2done) {
                if (!overlayP2.querySelector(".scene-text--waiting")) {
                    overlayP2.innerHTML =
                        '<p class="scene-text scene-text--waiting">Waiting for player 1...</p>';
                }
            }
            ...
            if (
                (g1 === "Thumb_Down" && c1 >= 0.7) ||
                (g2 === "Thumb_Down" && c2 >= 0.7) ||
                (g3 === "Thumb_Down" && c3 >= 0.7) ||
                (g4 === "Thumb_Down" && c4 >= 0.7)
            ) {
                disableMultiplayer();
                overlayP1.innerHTML = "";
                overlayP2.innerHTML = "";
        ...
        if (
            step < steps.length &&
            overlay.dataset.countdownStep !== String(step)
        ) {
        ...
        if (gameMode === "single") {
            ...
            if (timeLeft <= 5) {
                ...
                const el = overlay.querySelector(
                    ".scene-text--game-time-countdown"
                );
        ...
        if (gameMode === "multi") {
            ...
            if (overlay.querySelector(".scoreboard__playagain")) {
                overlay.querySelector(
                    ".scoreboard__playagain"
                ).style.visibility = bufferTime ? "hidden" : "visible";
            }
