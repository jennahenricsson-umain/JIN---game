...
function runRecognizer(recognizer, input, playerIndex, ctx) {
    ...
    for (let handIndex = 0; handIndex < 2; handIndex++) {
        ...
        handedness[playerIndex][handIndex] =
            handResult?.[0]?.categoryName ?? "";

        if (landmarks?.length > 10) {
            handX[playerIndex][handIndex] = toScreenX(
                landmarks[9].x,
                playerIndex,
                vw,
                scale,
            ...
            handY[playerIndex][handIndex] =
                landmarks[9].y * vh * scale - offsetY;

            // Draw a transparent rectangle around the hand to indicate detection
            ctx.fillStyle =
                handedness[playerIndex][handIndex] === "Left"
...
export function getHandPosition(playerIndex = 0, handIndex = 0) {
    return {
        x: handX[playerIndex][handIndex],
        y: handY[playerIndex][handIndex],
    };
}
